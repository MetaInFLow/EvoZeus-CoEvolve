...
`--actor` and `--permission` are expectations. Live Core evidence controls the result. Direct requires a writable, non-archived, non-disabled Repo, and every effective origin fetch/push URL must resolve to the declared exact GitHub Repo. Missing or partial permission evidence resolves the permission path to local; unavailable or invalid Issue evidence blocks the whole plan. A dirty checkout, wrong base, branch/worktree collision, unreconfirmed stale ledger ownership, or a canonical-checkout path also returns blockers. Do not modify business files while any blocker remains.
