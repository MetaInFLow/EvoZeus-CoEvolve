...
5. If this is a runtime-only install, do not treat the install copy as source; record the fallback and repair the canonical repo.
...
4. When the user reports an unsatisfactory result, run feedback audit and show `🧙🏻‍♂️ [EvoZeus][进化信号已捕获｜本地待确认｜<signal-id>]`; continue the business task without external writes.
