...
`SKILL.md` starts with `EvoZeus-CoEvolve 状态检查` immediately after frontmatter. Run that check before the target Skill's main chain:
...
When EvoZeus-CoEvolve publishes a newer harness version, do not rewrite target Skill business rules. Generate an append-only migration plan from the EvoZeus-CoEvolve repo:
