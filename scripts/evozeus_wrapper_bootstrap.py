...
1. 受信任的 global prompt watcher 只把普通 Chat 轮次接入 Core-owned runtime，无需先 `@Skill`。Core 读取注册目标并执行已验证的 Session Signal 方法；先完成当前业务纠正，只在回复末尾展示自然语言 Lesson Notice。需要准备 Issue 时再运行 feedback audit。Audit JSON 与内部字段不得展示。只有明确确认后才创建 Issue，修复继续要求单独授权。
...
- `global_prompt_lesson_watcher`：user-level `UserPromptSubmit` 只负责 Core runtime 的 Hook 生命周期；Core 负责目标 inventory、可信组件发现与 fail-open transport。该能力不自动记录，也不证明精确 Skill invocation。
