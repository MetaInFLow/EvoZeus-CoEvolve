...
def _read_regular_source(path: str) -> bytes:
    ...
    try:
        ...
        raise ImportError(f"trusted source is unavailable: {path}") from exc
    try:
        metadata = os.fstat(descriptor)
        if not stat.S_ISREG(metadata.st_mode):
            raise ImportError(f"trusted source is not a regular file: {path}")
        if metadata.st_size < 1 or metadata.st_size > _MAX_SOURCE_BYTES:
            raise ImportError(f"trusted source size is invalid: {path}")
        chunks: list[bytes] = []
        remaining = metadata.st_size
        while remaining:
            chunk = os.read(descriptor, min(remaining, 1024 * 1024))
            if not chunk:
                raise ImportError(f"trusted source changed while reading: {path}")
            chunks.append(chunk)
            remaining -= len(chunk)
        if os.read(descriptor, 1):
            raise ImportError(f"trusted source grew while reading: {path}")
        ...
        if (
            ...
            raise ImportError(f"trusted source metadata changed while reading: {path}")
...
class _EvoZeusSourceLoader:
    ...
    def exec_module(self, module: types.ModuleType) -> None:
        source_path = module.__spec__.origin if module.__spec__ else None
        if not isinstance(source_path, str):
            raise ImportError("trusted source module lacks an origin")
        source = _read_regular_source(source_path)
...
def bootstrap(
    ...
    scripts_dir = os.path.realpath(os.path.dirname(os.path.abspath(script_file)))
    repository_root = os.path.realpath(os.path.dirname(scripts_dir))
    cwd = os.path.realpath(os.getcwd())
    unsafe_roots = {scripts_dir, repository_root, cwd}
    clean_path: list[str] = []
    ...
    for name in tuple(sys.modules):
        if name.startswith("scripts.evozeus_") or (
            ...
            raise RuntimeError(f"local EvoZeus module was preloaded before trust bootstrap: {name}")
    ...
    loader = _EvoZeusSourceLoader(scripts_dir)
