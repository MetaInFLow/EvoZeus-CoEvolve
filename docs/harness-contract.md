...
- source 变更与新 closure 位于同一 PR 时，`construction_revision` 必须是最终 Release Commit 的可达祖先，且该 PR 仅允许 merge Commit；squash/rebase 会使单 PR provenance 失效并阻断 CI、UAT 晋级与 Release。若仓库只允许 squash/rebase，则先单独合并 source 变更取得 canonical reachable Commit，再在后续 data-only PR 冻结 closure 与 profiles。
